        ...
        while True:
            ...
            if richiesta == "DATE_ESAMI":
                ...
                if esame_trovato:
                    ...
                    conn.sendall(pickle.dumps(esame_trovato))
                else:
                    errore = "Esame non trovato"
                    # Se l'esame non è trovato, invia un messaggio di errore
                    conn.sendall(pickle.dumps(errore))
...
def visualizza_esami():
    ...
    if os.path.exists(FILE_ESAMI):
        ...
        with open(FILE_ESAMI, 'rb') as f:
            ...
            for esame in Lista_esami:
                print(
                    f"Nome Esame: {esame.nome_esame}, "
                    f"Date Disponibili: {', '.join(esame.date_disponibili)}"
                )
    else:
